...
public class MultipartUploadDemo {
    ...
    private static String secretId = "AKIDgAPPDsoGvLWSCkECC1ShhS1MSjnlq5Vl";
    private static String secretKey = "IMwKYZOyPdjcR9lFgspW3LVbXIo6C2bx";
    private static String region = "ap-shanghai";
    private static String bucketName = "wukong-1257667045";
    private static String scheme = "https";

    public static String InitMultipartUploadDemo(String packagePath) {
...
//        String key = "aaa/bbb.txt";
        InitiateMultipartUploadRequest request = new InitiateMultipartUploadRequest(bucketName, packagePath);
    ...
    public static List<PartETag> listPartDemo(String uploadId,String packagePath) {
        // 1 初始化用户身份信息(secretId, secretKey)
        COSCredentials cred = new BasicCOSCredentials(secretId, secretKey);
        // 2 设置bucket的区域, COS地域的简称请参照 https://www.qcloud.com/document/product/436/6224
        ClientConfig clientConfig = new ClientConfig(new Region(region));
        ...
        ListPartsRequest listPartsRequest = new ListPartsRequest(bucketName, packagePath, uploadId);
        do {
            try {
    ...
    // 分块上传(上传某一个分片的数据)
    public static List<PartETag> UploadPartDemo(String uploadId,String packagePath)  {
        // 1 初始化用户身份信息(secretId, secretKey)
        COSCredentials cred = new BasicCOSCredentials(secretId, secretKey);
        // 2 设置bucket的区域, COS地域的简称请参照 https://www.qcloud.com/document/product/436/6224
        ClientConfig clientConfig = new ClientConfig(new Region(region));
        ...
        for (int i = 0; i < 10; i++) {
            ...
            uploadPartRequest.setKey(packagePath);
    ...
    // complete完成分片上传
    public static void completePartDemo(String uploadId, List<PartETag> partETags, String packagePath) {
        // 1 初始化用户身份信息(secretId, secretKey)
        COSCredentials cred = new BasicCOSCredentials(secretId, secretKey);
        // 2 设置bucket的区域, COS地域的简称请参照 https://www.qcloud.com/document/product/436/6224
        ClientConfig clientConfig = new ClientConfig(new Region(region));
        ...
        CompleteMultipartUploadRequest completeMultipartUploadRequest =
                new CompleteMultipartUploadRequest(bucketName, packagePath, uploadId, partETags);
        try {
    ...
    public static void main(String[] args) {
        String uploadId = InitMultipartUploadDemo("task.txt");
        List<PartETag> partETags = UploadPartDemo(uploadId,"task.txt");
        completePartDemo(uploadId, partETags,"task.txt");
    }
}